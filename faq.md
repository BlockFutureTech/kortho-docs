# 使用什么 sdk
KroTho 高度兼容 ethereum，可以使用 ethereum 的 sdk，例如 web3js， web3j 等。
...
# 资产如何进入科图生态链KroTho？
目前已支持HT资产，HT在Global对科图生态链KroTho的充提于12月10日上线；ETH、BTC、HUSD、DOT、FIL、BCH、LTC等资产于12月20日上链。

# 有类似swap的Dex合约提供换币吗？
目前官方Dex还没有，鼓励社区开发者提供。

# 是否有跨链桥接（bridge）资产到科图生态链KroTho？
...
# 是否支持--pruning 参数
`--pruning`是 parity 的参数，KroTho 的 API 遵循 go-ethereum 的风格，可以使用 `--gcmode archive` 来实现相同的功能。
